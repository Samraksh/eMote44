...
DeviceStatus OMACType::SetOMACParametersBasedOnRadioName(UINT8 radioName){
	...
	case SX1276RADIO:
		{
			UINT16 TX_BUFFER = 20 * MILLISECINMICSEC; // 10351
			UINT16 RX_BUFFER = 70 * MILLISECINMICSEC;
			UINT16 DISCO_BUFFER = 10 * MILLISECINMICSEC; // 7031
			...
			MAX_PACKET_TX_DURATION_MICRO = 41000;//((UINT32)IEEE802_15_4_FRAME_LENGTH*(UINT32)BITS_PER_BYTE*(UINT32)TX_TIME_PER_BIT_IN_MICROSEC) + TX_BUFFER;
			MAX_PACKET_RX_DURATION_MICRO = 30848;//46208;//MAX_PACKET_TX_DURATION_MICRO;
			...
			DISCO_PACKET_TX_TIME_MICRO = 62000;//(sizeof(DiscoveryMsg_t)*BITS_PER_BYTE*(UINT32)TX_TIME_PER_BIT_IN_MICROSEC) + DISCO_BUFFER;	//10*MILLISECINMICSEC;
			DISCO_BEACON_TX_MAX_DURATION_MICRO = 10*MILLISECINMICSEC;
			DISCO_SLOT_PERIOD_MICRO = DISCOPERIODINMILLI *MILLISECINMICSEC;
			HIGH_DISCO_PERIOD_IN_SLOTS = HIGH_DISCO_PERIOD_IN_SLOTS_CONSTANT;
			if(CPU_Radio_GetRadioAckType() == SOFTWARE_ACK){
				...
				RETRANS_DELAY_DUE_TO_MISSING_HW_ACK = 0*MILLISECINMICSEC;
				RETRANS_DELAY_DUE_TO_MISSING_SW_ACK = 0*MILLISECINMICSEC;
				ACK_TX_MAX_DURATION_MICRO = 25488;//25839;//MAX_PACKET_TX_DURATION_MICRO; //100*(UINT32)MILLISECINMICSEC;;
			}
			else if(CPU_Radio_GetRadioAckType() == HARDWARE_ACK){
				ASSERT_SP(0);
			}
