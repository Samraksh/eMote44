	...
	case SX1276RADIO:
		{
			...
			MAX_PACKET_TX_DURATION_MICRO = 120000;//544000;//56559;//((UINT32)IEEE802_15_4_FRAME_LENGTH*(UINT32)BITS_PER_BYTE*(UINT32)TX_TIME_PER_BIT_IN_MICROSEC) + TX_BUFFER;
			MAX_PACKET_RX_DURATION_MICRO = 120000;//544000;//46208;//MAX_PACKET_TX_DURATION_MICRO;
			ACK_RX_MAX_DURATION_MICRO = MAX_PACKET_TX_DURATION_MICRO;//15488;// //(sizeof(softwareACKHeader)*BITS_PER_BYTE*(UINT32)TX_TIME_PER_BIT_IN_MICROSEC) + RX_BUFFER;	//8*MILLISECINMICSEC;
			DISCO_PACKET_TX_TIME_MICRO = 72000;//(sizeof(DiscoveryMsg_t)*BITS_PER_BYTE*(UINT32)TX_TIME_PER_BIT_IN_MICROSEC) + DISCO_BUFFER;	//10*MILLISECINMICSEC;
			DISCO_BEACON_TX_MAX_DURATION_MICRO = 10*MILLISECINMICSEC;
			DISCO_SLOT_PERIOD_MICRO = DISCOPERIODINMILLI *MILLISECINMICSEC;
			HIGH_DISCO_PERIOD_IN_SLOTS = HIGH_DISCO_PERIOD_IN_SLOTS_CONSTANT;
			if(CPU_Radio_GetRadioAckType() == SOFTWARE_ACK){
...
Message_15_4_t* OMACType::ReceiveHandler(Message_15_4_t* msg, int Size){
	...
	if(sourceID == Neighbor2beFollowed) {
		...
		if(msg->GetHeader()->flags & TIMESTAMPED_FLAG){
			...
			//////
			//////hal_printf("Received Packet: %llu, %llu, %llu\r\n",g_OMAC.m_Clock.GetCurrentTimeinTicks(), HAL_Time_CurrentTicks(), msg->GetMetaData()->GetReceiveTimeStamp());
