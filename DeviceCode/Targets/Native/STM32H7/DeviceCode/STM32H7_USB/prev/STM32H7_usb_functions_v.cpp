...
#if !defined(STM32H7_USB_FS_USE_ID_PIN)
#define STM32H7_USB_FS_USE_ID_PIN 0
#endif
#if !defined(STM32H7_USB_FS_USE_VB_PIN)
#define STM32H7_USB_FS_USE_VB_PIN 0
#endif
#if !defined(STM32H7_USB_HS_USE_ID_PIN)
#define STM32H7_USB_HS_USE_ID_PIN 0
#endif
#if !defined(STM32H7_USB_HS_USE_VB_PIN)
#define STM32H7_USB_HS_USE_VB_PIN 0
#endif


#define MAX_EP0_SIZE 64     // maximum control channel packet size
#define DEF_EP0_SIZE 8      // default control channel packet size
#define STM32H7_Num_EP_FS 6 // OTG FS supports 6 endpoints
#define STM32H7_Num_EP_HS 9 // OTG FS supports 9 endpoints
...
#define STM32H7_USB_REGS(c) (c ? OTG_HS : OTG_FS)

#define STM32H7_USB_IS_HS(c) c
#define STM32H7_USB_IDX(c) c
#define STM32H7_USB_FS_IDX 0
#define STM32H7_USB_HS_IDX 1

#define STM32H7_USB_DM_PIN(c) (c ? 30 : 11) // B14,A11
#define STM32H7_USB_DP_PIN(c) (c ? 31 : 12) // B15,A12
#define STM32H7_USB_VB_PIN(c) (c ? 29 :  9) // B13,A9
#define STM32H7_USB_ID_PIN(c) (c ? 28 : 10) // B12,A10

#define STM32H7_USB_USE_ID_PIN(c) (c ? STM32H7_USB_HS_USE_ID_PIN : STM32H7_USB_FS_USE_ID_PIN)
#define STM32H7_USB_USE_VB_PIN(c) (c ? STM32H7_USB_HS_USE_VB_PIN : STM32H7_USB_FS_USE_VB_PIN)
#define STM32H7_USB_ALT_MODE(c) (GPIO_ALT_MODE)(c ? 0x2C2 : 0x2A2) // AF12,AF10, 50MHz

#if USB_MAX_QUEUES <= STM32H7_Num_EP_FS
#define USB_MAX_EP(c) USB_MAX_QUEUES
#define USB_MAX_BUFFERS (2 * USB_MAX_QUEUES - 2)
#elif USB_MAX_QUEUES <= STM32H7_Num_EP_HS
#define USB_MAX_EP(c) (c ? USB_MAX_QUEUES : STM32H7_Num_EP_FS)
#define USB_MAX_BUFFERS (USB_MAX_QUEUES + STM32H7_Num_EP_FS - 2)
#else
#define USB_MAX_EP(c) (c ? STM32H7_Num_EP_HS : STM32H7_Num_EP_FS)
#define USB_MAX_BUFFERS (STM32H7_Num_EP_HS + STM32H7_Num_EP_FS - 2)
#endif


#elif defined(STM32H7_USB_HS)
// use USB OTG High Speed

#define STM32H7_USB_REGS(c) OTG_HS

#define STM32H7_USB_IS_HS(c) TRUE
#define STM32H7_USB_IDX(c) 0
#define STM32H7_USB_FS_IDX 0
#define STM32H7_USB_HS_IDX 0

#define STM32H7_USB_DM_PIN(c) 30 // B14
#define STM32H7_USB_DP_PIN(c) 31 // B15
#define STM32H7_USB_VB_PIN(c) 29 // B13
#define STM32H7_USB_ID_PIN(c) 28 // B12

#define STM32H7_USB_USE_ID_PIN(c) STM32H7_USB_HS_USE_ID_PIN
#define STM32H7_USB_USE_VB_PIN(c) STM32H7_USB_HS_USE_VB_PIN
#define STM32H7_USB_ALT_MODE(c) (GPIO_ALT_MODE)0x2C2; // AF12, 50MHz

#if USB_MAX_QUEUES <= STM32H7_Num_EP_HS
#define USB_MAX_EP(c) USB_MAX_QUEUES
#define USB_MAX_BUFFERS (USB_MAX_QUEUES - 1)
#else
#define USB_MAX_EP(c) STM32H7_Num_EP_HS
#define USB_MAX_BUFFERS (STM32H7_Num_EP_HS - 1)
#endif
...
#define STM32H7_USB_REGS(c) OTG_FS

#define STM32H7_USB_IS_HS(c) FALSE
#define STM32H7_USB_IDX(c) 0
#define STM32H7_USB_FS_IDX 0
#define STM32H7_USB_HS_IDX 0

#define STM32H7_USB_DM_PIN(c) 11 // A11
#define STM32H7_USB_DP_PIN(c) 12 // A12
#define STM32H7_USB_VB_PIN(c)  9 // A9
#define STM32H7_USB_ID_PIN(c) 10 // A10

#define STM32H7_USB_USE_ID_PIN(c) STM32H7_USB_FS_USE_ID_PIN
#define STM32H7_USB_USE_VB_PIN(c) STM32H7_USB_FS_USE_VB_PIN
#define STM32H7_USB_ALT_MODE(c) (GPIO_ALT_MODE)0x2A2; // AF10, 50MHz

#if USB_MAX_QUEUES <= STM32H7_Num_EP_FS
#define USB_MAX_EP(c) USB_MAX_QUEUES
#define USB_MAX_BUFFERS (USB_MAX_QUEUES - 1)
#else
#define USB_MAX_EP(c) STM32H7_Num_EP_FS
#define USB_MAX_BUFFERS (STM32H7_Num_EP_FS - 1)
#endif
...
#define STM32H7_USB_TRDT ((4 * 48000000 - 1) / SYSTEM_CYCLE_CLOCK_HZ + 2)
...
static STM32H7_USB_STATE STM32H7_USB_ControllerState[ TOTAL_USB_CONTROLLER ];
 ...
 */
void STM32H7_USB_Driver_SuspendEvent( OTG_TypeDef* OTG, USB_CONTROLLER_STATE* State )
{
	...
	hal_printf(" USB_SuspendEvent ");
	
    ( ( STM32H7_USB_STATE* )State )->previousDeviceState = State->DeviceState;
 ...
 */
void STM32H7_USB_Driver_ResumeEvent( OTG_TypeDef* OTG, USB_CONTROLLER_STATE* State )
{
    hal_printf(" USB_ResumeEvent ");

    OTG->DCTL &= ~OTG_DCTL_RWUSIG; // remove remote wakeup signaling

    State->DeviceState = ( ( STM32H7_USB_STATE* )State )->previousDeviceState;
    USB_StateCallback( State );
}
 ...
 */
void STM32H7_USB_Driver_ResetEvent( OTG_TypeDef* OTG, USB_CONTROLLER_STATE* State )
{
    hal_printf(" USB_ResetEvent ");
    ...
    UINT32 eptype = ( ( STM32H7_USB_STATE* )State )->EP_Type >> 2; // endpoint types (2 bits / endpoint)
    UINT32 i = 1, bit = 2;
    while( eptype )
    {
 ...
 */
void STM32H7_USB_Driver_EP_RX_Int( OTG_TypeDef* OTG
                                 , USB_CONTROLLER_STATE* State
                                 , UINT32 ep
                                 , UINT32 count
                                 )
{
    ...
    ////hal_printf(" USB_Rx interrupt ");
    
	if( ep == 0 )
    { // control endpoint
        USB_Debug( count ? 'c' : '0' );

        pd = ( UINT32* )( ( STM32H7_USB_STATE* )State )->ep0Buffer;
        State->Data = ( BYTE* )pd;
 ...
 */
void STM32H7_USB_Driver_EP_In_Int( OTG_TypeDef* OTG, USB_CONTROLLER_STATE* State, UINT32 ep )
{
    ASSERT_IRQ_MUST_BE_OFF( );
 ...
 */
void STM32H7_USB_Driver_Handle_Setup( OTG_TypeDef* OTG, USB_CONTROLLER_STATE* State )
{
    ...
    STM32H7_USB_Driver_EP_In_Int( OTG, State, 0 );
    ...
    if( result == USB_STATE_CONFIGURATION )
    {
        for( int ep = 1; ep < State->EndpointCount; ep++ )
        {
            ...
            {
                STM32H7_USB_Driver_EP_In_Int( OTG, State, ep );
            }
        }
    }
}
 ...
 */
void STM32H7_USB_Driver_EP_Out_Int( OTG_TypeDef* OTG, USB_CONTROLLER_STATE* State, UINT32 ep )
{
    ASSERT_IRQ_MUST_BE_OFF( );
    ...
    { // control endpoint
        ...
        STM32H7_USB_Driver_Handle_Setup( OTG, State );
    }
 ...
 */
void STM32H7_USB_Driver_Interrupt( OTG_TypeDef* OTG, USB_CONTROLLER_STATE* State )
{
	////hal_printf(" 233323 ");
    INTERRUPT_START;
    ...
    { // RxFifo non empty
        ...
        { 
            STM32H7_USB_Driver_EP_RX_Int( OTG, State, ep, count );
        }
        else
        {
    ...
    { // OUT endpoint
        ...
        {
            if( bits & 1 )
                STM32H7_USB_Driver_EP_Out_Int( OTG, State, ep );
    ...
    { // reset
        STM32H7_USB_Driver_ResetEvent( OTG, State );
    ...
    else
    {
        ...
        { // suspend
            STM32H7_USB_Driver_SuspendEvent( OTG, State );
            OTG->GINTSTS = OTG_GINTSTS_USBSUSP; // clear interrupt
        }

        if( intPend & OTG_GINTSTS_WKUPINT )
        { // wakeup
            STM32H7_USB_Driver_ResumeEvent( OTG, State );
 ...
 */
void STM32H7_USB_Driver_FS_Interrupt( void* param )
{
    STM32H7_USB_Driver_Interrupt( OTG_FS, &STM32H7_USB_ControllerState[ STM32H7_USB_FS_IDX ].state );
}

/*
 * OTG HS Interrupt Handler
 */
void STM32H7_USB_Driver_HS_Interrupt( void* param )
{
    STM32H7_USB_Driver_Interrupt( OTG_HS, &STM32H7_USB_ControllerState[ STM32H7_USB_HS_IDX ].state );
}
...
USB_CONTROLLER_STATE * CPU_USB_GetState( int Controller )
{
    ...
    return &STM32H7_USB_ControllerState[ STM32H7_USB_IDX( Controller ) ].state;
}
...
HRESULT CPU_USB_Initialize( int Controller )
{
	...
	// enable USB clock
    if( STM32H7_USB_IS_HS( Controller ) )
    { // HS on AHB1
    ...
    USB_CONTROLLER_STATE *State = &STM32H7_USB_ControllerState[ STM32H7_USB_IDX( Controller ) ].state;

    OTG_TypeDef* OTG = STM32H7_USB_REGS( Controller );
	...
	//while ((OTG->GRSTCTL & USB_OTG_GRSTCTL_CSRST) == USB_OTG_GRSTCTL_CSRST);
	
	/* Deactivate the power down*/
    OTG->GCCFG = USB_OTG_GCCFG_PWRDWN;

	/* Set functional mode : Device mode */
	OTG->GUSBCFG &= ~(USB_OTG_GUSBCFG_FHMOD | USB_OTG_GUSBCFG_FDMOD); 
...
// UNDONE: FIXME:
    if( STM32H7_USB_USE_VB_PIN( Controller ))
	{
        OTG->GCCFG |= OTG_GCCFG_VBDEN;         // enable vbus sense
	}
    if (!STM32H7_USB_USE_ID_PIN(Controller)) { // no ID pin
    ...
    ( ( STM32H7_USB_STATE* )State )->pinsProtected = TRUE;
    State->EndpointStatus = ( ( STM32H7_USB_STATE* )State )->endpointStatus;
    ...
    UINT32 queueIdx = STM32H7_USB_IDX( Controller ) ? USB_MAX_EP( 0 ) - 1 : 0; // first queue buffer
    ...
    ( ( STM32H7_USB_STATE* )State )->EP_Type = epType;
    ...
    if( STM32H7_USB_IS_HS( Controller ) )
    { // HS
        CPU_INTC_ActivateInterrupt( OTG_HS_IRQn, STM32H7_USB_Driver_HS_Interrupt, 0 );
        CPU_INTC_ActivateInterrupt( OTG_HS_WKUP_IRQn, STM32H7_USB_Driver_HS_Interrupt, 0 );
    }
    else
    { // FS
        CPU_INTC_ActivateInterrupt( OTG_FS_IRQn, STM32H7_USB_Driver_FS_Interrupt, 0 );
        CPU_INTC_ActivateInterrupt( OTG_FS_WKUP_IRQn, STM32H7_USB_Driver_FS_Interrupt, 0 );
    }
...
BOOL CPU_USB_StartOutput( USB_CONTROLLER_STATE* State, int ep )
{
    ...
    OTG_TypeDef* OTG = STM32H7_USB_REGS( State->ControllerNum );
    ...
    { // check all endpoints for pending actions
        STM32H7_USB_Driver_Interrupt( OTG, State );
    }
...
BOOL CPU_USB_ProtectPins( int Controller, BOOL On )
{
    ...
    if( ( ( STM32H7_USB_STATE* )State )->EP_Type == 0 )
    ...
    if( On )
    {
        if( !( ( STM32H7_USB_STATE* )State )->pinsProtected )
        {
            ////hal_printf(" + ");
    ...
    else
    {
        if( ( ( STM32H7_USB_STATE* )State )->pinsProtected )
        {
            ...
            ( ( STM32H7_USB_STATE* )State )->pinsProtected = FALSE;

            GPIO_ALT_MODE altMode = STM32H7_USB_ALT_MODE( Controller );
            CPU_GPIO_DisablePin( STM32H7_USB_DM_PIN( Controller ), RESISTOR_DISABLED, 2, altMode );
            CPU_GPIO_DisablePin( STM32H7_USB_DP_PIN( Controller ), RESISTOR_DISABLED, 2, altMode );
            if( STM32H7_USB_USE_ID_PIN( Controller ) )
            {
                CPU_GPIO_DisablePin( STM32H7_USB_ID_PIN( Controller ), RESISTOR_DISABLED, 2, altMode );
            }
			//CPU_GPIO_DisablePin( STM32H7_USB_VB_PIN( Controller ), RESISTOR_DISABLED, 0, altMode );
