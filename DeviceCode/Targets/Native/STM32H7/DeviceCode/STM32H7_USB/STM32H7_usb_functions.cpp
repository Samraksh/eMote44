...
BOOL CPU_USB_ProtectPins( int Controller, BOOL On )
{
    ...
    else
    {
        if( ( ( STM32H7_USB_STATE* )State )->pinsProtected )
        {
            ...
            CPU_GPIO_DisablePin( STM32H7_USB_DM_PIN( Controller ), RESISTOR_DISABLED, 0, altMode );
            CPU_GPIO_DisablePin( STM32H7_USB_DP_PIN( Controller ), RESISTOR_DISABLED, 0, altMode );
            if( STM32H7_USB_USE_ID_PIN( Controller ) )
            {
                CPU_GPIO_DisablePin( STM32H7_USB_ID_PIN( Controller ), RESISTOR_DISABLED, 0, altMode );
            }
