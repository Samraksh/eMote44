...
#define GPIO_0 (GPIO_PIN)120 //_P(A,4)
#define GPIO_1 (GPIO_PIN)120 //_P(C,10)
...
void Timer_RTC_Handler(void *arg)
{
	...
	if (state)
		...
		state = TRUE;
	CPU_GPIO_SetPinState(GPIO_0, state);
	uint64_t time = CPU_Timer_CurrentTicks(SYSTEM_TIME);
	hal_printf("SYSTEM: %llu\r\n", time);
}
...
void Timer_1_Handler(void *arg)
{
	...
	if (state)
		...
		state = TRUE;
	CPU_GPIO_SetPinState(GPIO_1, state);
	uint64_t time = CPU_Timer_CurrentTicks(LPTIM);
	hal_printf("LPTIM: %llu\r\n", time);
}
...
void ApplicationEntryPoint()
{
   ...
   // CPU_GPIO_EnableOutputPin(LED3, TRUE);
	...
	//VirtTimer_SetTimer(VIRT_TIMER_LED_GREEN, 0, 500000, FALSE, FALSE, Timer_1_Handler);
	//VirtTimer_SetTimer(VIRT_TIMER_LED_GREEN, 0, 500000, FALSE, FALSE, Timer_Green_Handler);
	//VirtTimer_Start(VIRT_TIMER_LED_GREEN);
	//VirtTimer_SetTimer(VIRT_TIMER_LED_RED, 0, 500000, FALSE, FALSE, Timer_RTC_Handler, LOW_DRIFT_TIMER);
...
*/
	...
	//VirtTimer_SetTimer(VIRT_TIMER_TIME_TEST, 0, 1000000, FALSE, FALSE, Timer_Green_Handler); 
	//VirtTimer_Start(VIRT_TIMER_TIME_TEST);
	//VirtTimer_SetTimer(VIRT_TIMER_RTC_TEST, 0, 1000000, FALSE, FALSE, Timer_RTC_Handler, LOW_DRIFT_TIMER);
	//VirtTimer_SetTimer(VIRT_TIMER_RTC_TEST, 0, 1000000, FALSE, FALSE, Timer_Red_Handler, LOW_DRIFT_TIMER);
